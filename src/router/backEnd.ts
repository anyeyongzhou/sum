...
import Personsl from "/@/views/personal/index.vue";
...
export async function initBackEndControlRoutes() {
  ...
  // 获取路由菜单数据
  let res = await getBackEndControlRoutes();
  processNames(res.data);
  // const res = await useLoginApis.getMenu({});
...
function processNames(data: any) {
  data.forEach((item: any) => {
    ...
    // 如果当前节点有 children，则递归处理子节点
    if (item.children && Array.isArray(item.children)) {
      processNames(item.children);
    }
  });
}
