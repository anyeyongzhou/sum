	...
	methods: {
		...
		run() {
			...
			that.animationFrame = window.requestAnimationFrame(() => {
				...
				//判断滚动值
				if (!that.isStop) {
					if (that.scrollDirection === 'top' || that.scrollDirection === 'left') {
						that.scrollDistance -= that.steep;
					} else if (that.scrollDirection === 'bottom' || that.scrollDirection === 'right') {
						that.scrollDistance += that.steep;
					}
					that.run();
				}
		...
		initData() {
			this.$nextTick(() => {
				...
				if (
					(this.bodyHeight !== 0 && this.listHeight !== 0 && this.listHeight >= this.bodyHeight) ||
