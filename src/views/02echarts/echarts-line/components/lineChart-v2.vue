      ...
      if (this.colorGroup) {
        this.colorGroup.forEach((item, index, array) => {
          this.colorGroupRgb.push(
            parseInt("0x" + item.slice(1, 3)) +
              "," +
              parseInt("0x" + item.slice(3, 5)) +
              "," +
              parseInt("0x" + item.slice(5, 7))
          );
        });
      }
    },
          ...
          formatter: value => {
            ...
            if (value.length > 0) {
              ...
              if (this.xAxisData1 && this.xAxisData1.length > 0) {
                strHtml = `<div><div style="font-weight:800">${
                  this.xAxisData1[value[0].dataIndex]
                }</div>`;
              } else {
                if (value[0].axisValueLabel.length > this.wrapMaxlength) {
                  labelTemp =
                    String(value[0].axisValueLabel).slice(
                      0,
                      this.wrapMaxlength
                    ) +
                    "-" +
